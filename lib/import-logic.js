...
async function runImport() {
    ...
    if (privateKey.includes(header) && privateKey.includes(footer)) {
        // 1. Limpiamos TODO lo que no sea un carácter base64 válido
        // Importante: .replace(/\s/g, '') NO quita '\n' literales (dos caracteres)
        const base64Body = privateKey
            .replace(header, '')
            .replace(footer, '')
            .replace(/[^A-Za-z0-9+/=]/g, ''); // Nuclear: Solo permite base64

        // 2. Re-envolvemos a 64 caracteres
