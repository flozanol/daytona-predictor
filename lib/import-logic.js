...
async function runImport() {
    ...
    // 1. Limpieza inicial de comillas y escapes de Vercel
    privateKey = privateKey.replace(/^"|"$/g, '').replace(/\\n/g, '\n');

    // 2. Reconstrucción Súper Robusta del PEM
    const header = '-----BEGIN PRIVATE KEY-----';
    const footer = '-----END PRIVATE KEY-----';

    if (privateKey.includes(header) && privateKey.includes(footer)) {
        // Extraemos solo el contenido base64 quitando encabezados y cualquier espacio/salto
        const base64Body = privateKey
            .replace(header, '')
            .replace(footer, '')
            .replace(/\s/g, ''); // Quitamos TODO espacio, tab y salto

        // Re-envolvemos a 64 caracteres (estándar PEM) para máxima compatibilidad
